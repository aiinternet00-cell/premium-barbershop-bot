...
WORK_HOURS = {0: (10, 21), 1: (10, 21), 2: (10, 21), 3: (10, 21), 4: (10, 21), 5: (11, 20), 6: (11, 20)}
...
def load_settings() -> Settings:
    ...
    if not token or not admin:
        raise RuntimeError("Обязательные переменные BOT_TOKEN и ADMIN_ID не заданы. См. .env.example")
    try:
        admin_id = int(admin)
    except ValueError as exc:
        raise RuntimeError("ADMIN_ID должен быть целым числом") from exc
    return Settings(token, admin_id, os.getenv("DATABASE_PATH", "barbershop.db"))
